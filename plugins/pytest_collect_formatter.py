...
# This plugin is based on the pytest_collect_formatter plugin, just modified to support python 2.7
# https://github.com/LaserPhaser/pytest-collect-formatter
...
def check_parent(item, item_data):
    if type(item).__name__ not in ["Session", "Instance"]:
        item_data = {hash(type(item).__name__ + item.name): {"type": type(item).__name__, "title": item.name,
                                                             "children": item_data}}
